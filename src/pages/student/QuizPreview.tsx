...
const mockQuizzes: { [key: string]: Quiz } = {
  '1': {
    ...
    description: 'اختبار شامل حول مفاهيم إدارة المخازن والتخزين في اللوجستيات',
...
const QuizPreview = () => {
    ...
    return (
      <LEMSLayout userRole="student">
        <div className="max-w-4xl mx-auto text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">الاختبار المطلوب غير متاح</p>
          <Button asChild>
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case 'سهل': return 'text-success bg-success/10';
      case 'متوسط': return 'text-warning bg-warning/10';
      case 'صعب': return 'text-destructive bg-destructive/10';
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'تدريبي': return 'text-success bg-success/10';
      case 'تقييمي': return 'text-warning bg-warning/10';
      case 'نهائي': return 'text-destructive bg-destructive/10';
      default: return 'text-muted-foreground bg-muted/10';
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="lems-card text-center p-4">
            ...
            <div className="text-sm text-muted-foreground">الحد الأدنى للنجاح</div>
          </Card>
        ...
        <Card className="lems-card">
          <div className="space-y-4">
            ...
            <ul className="space-y-3 text-sm">
              ...
              <li className="flex items-start gap-2">
                <span className="text-primary">•</span>
                <span>يمكنك التنقل بين الأسئلة باستخدام أزرار التالي والسابق</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-primary">•</span>
              ...
              <li className="flex items-start gap-2">
                <span className="text-primary">•</span>
                <span>سيتم إرسال الاختبار تلقائياً عند انتهاء الوقت المحدد ({quiz.duration} دقيقة)</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-primary">•</span>
                <span>تحتاج للحصول على {quiz.minimumScore}% على الأقل للنجاح</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-primary">•</span>
                <span>لديك {quiz.maxAttempts - quiz.attempts} محاولات متبقية</span>
              </li>
            </ul>
          </div>
        </Card>
        ...
        <Card className="lems-card">
          <div className="space-y-4">
            <h3 className="text-xl font-semibold">معاينة الأسئلة ({quiz.questions.length})</h3>
            <div className="space-y-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {question.type === 'multiple_choice' ? 'اختيار متعدد' : 
                         question.type === 'true_false' ? 'صح/خطأ' : 'ملء الفراغ'} • 
